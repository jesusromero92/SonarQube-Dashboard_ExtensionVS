...
| Remediación en vivo | Sigue las ediciones de los issues sincronizados, mantiene alineados sus rangos y puede usar diagnósticos independientes de SonarQube for IDE para marcarlos como corregidos localmente mientras esperan confirmación del servidor. |
...
- **Estado de remediación en vivo** que marca los defectos tocados como modificados localmente y, cuando SonarQube for IDE confirma de forma independiente que el diagnóstico ha desaparecido, los mantiene en Problems como entradas informativas corregidas localmente hasta que el siguiente análisis del servidor confirme el resultado.
...
- cuando ese diagnóstico Sonar externo desaparece después de la edición, el issue pasa a **Corregido localmente · pendiente de confirmación de SonarQube**, permanece visible en **Problems** como entrada informativa pendiente de confirmación, se excluye de la navegación normal de defectos y mantiene un marcador verde visible en el editor hasta que un nuevo análisis del repositorio confirme el estado del servidor;
- si el analizador externo vuelve a informar de un issue que estaba corregido localmente, vuelve a **Modificado localmente · pendiente de validación** hasta que un nuevo análisis del repositorio confirme el estado del servidor;
- si SonarQube for IDE no está instalado, no está activo, no soporta el archivo o nunca informó de ese hallazgo exacto, la extensión mantiene deliberadamente **Modificado localmente · pendiente de validación** en lugar de asumir que está corregido;
...
- los estados pendientes **Modificado localmente** y **Corregido localmente** se guardan por workspace, incluido su rango seguido, de modo que al recargar o reiniciar VS Code se restaura el mismo estado pendiente hasta que un análisis del repositorio confirme el resultado del servidor;
- cuando la ventana se restaura con un archivo ya activo, la extensión publica sus diagnósticos SonarQube mediante una actualización separada y retrasada para que el comportamiento nativo `problems.autoReveal` de VS Code pueda revelar ese archivo en **Problems** tras la sincronización inicial; las entradas corregidas localmente también se mantienen en el panel para que una confirmación pendiente no haga desaparecer el archivo activo.
...
El contenedor de la barra de actividad incorpora ahora una vista nativa independiente **Issues corregidos localmente**, al mismo nivel que **Ejecuciones del pipeline** y **Explorador de issues**. Allí se muestran todos los defectos corregidos localmente que todavía esperan confirmación del servidor, incluyendo su regla, archivo y línea seguida actual. Al seleccionar una entrada se abre directamente ese hallazgo pendiente en el editor. La lista utiliza el comportamiento nativo de vistas desplegables de VS Code en lugar de estar incrustada dentro del webview de Resumen.

Cuando un nuevo análisis del repositorio confirma uno o varios de esos defectos corregidos localmente, la notificación normal de análisis finalizado que aparece abajo a la derecha indica también cuántos han sido confirmados y los elimina de la vista nativa pendiente.

El acordeón **Integración con el editor** también indica automáticamente si la extensión oficial **SonarQube for IDE** (`SonarSource.sonarlint-vscode`) está detectada y activa. Es opcional: sin ella, los defectos editados permanecen como **Modificado localmente · pendiente de validación** hasta el siguiente análisis del repositorio; cuando está activa, únicamente sus diagnósticos independientes coincidentes pueden promoverlos a **Corregido localmente · pendiente de confirmación de SonarQube**.
...
Los issues Overall se publican como diagnósticos nativos de VS Code. Con la remediación en vivo activada, Problems refleja también el estado local conservador: los hallazgos tocados pasan a informativos y los que SonarQube for IDE confirma de forma independiente como desaparecidos permanecen visibles como entradas informativas **Corregido localmente** hasta que el siguiente análisis del repositorio confirme el estado del servidor.
...
- **Remediación en vivo:** `sonarQubeDashboard.liveRemediation.enabled` controla el seguimiento local de defectos modificados/corregidos en Problems y el editor. Está activado de forma predeterminada.
