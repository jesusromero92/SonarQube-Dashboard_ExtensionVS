...
| Pipeline steps, detected commands, templates, execution history, pending Live Remediation state/ranges, server-baseline snippets, remediation-session metadata/results, diagnostic state, and notification baselines | VS Code workspace state | Stored locally for the workspace. Execution history is limited to the latest 30 runs per analysis folder; Live Remediation pending state, minimal source snippets used for diff/safe revert, and remediation-session results are stored per workspace so they survive VS Code restarts; they are removed when Live Remediation state is explicitly cleared/disabled. |
...
| Pasos, comandos detectados, plantillas, historial, estados/rangos pendientes, fragmentos baseline de servidor, metadatos/resultados de la sesión de Live Remediation, diagnóstico y referencias de notificación | Estado del workspace de VS Code | Se guardan localmente para el workspace. El historial se limita a 30 ejecuciones por carpeta de análisis; el estado pendiente, los fragmentos mínimos usados para diff/revert seguro y los resultados de la sesión de Live Remediation se guardan por workspace para sobrevivir a reinicios de VS Code; se eliminan al limpiar/desactivar explícitamente el estado de Live Remediation. |
