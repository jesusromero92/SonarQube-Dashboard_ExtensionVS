...
- Added the module-owned **Configuration → Integrations** catalog. Detected tools can now be promoted explicitly into reusable **Pipeline steps**; they are not inserted directly into a selected template by this action. The extension host re-validates the integration before persisting the step, avoids duplicate commands, and the template editor consumes the resulting reusable step like any other custom step.

  *Se ha añadido el catálogo **Configuración → Integraciones** propiedad del módulo. Las herramientas detectadas pueden convertirse explícitamente en **pasos reutilizables de Pipeline**; esta acción ya no las inserta directamente en una plantilla concreta. El host de la extensión vuelve a validar la integración antes de persistir el paso, evita comandos duplicados y el editor de plantillas consume después ese paso reutilizable como cualquier otro paso personalizado.*
...
- Pipeline integrations now use an internal **Integration Provider API** under `src/modules/pipeline/integrations/`. Tool-specific detection, categories, failure policy, setup guidance, and watched configuration files are owned by independent providers and composed by a registry instead of being enumerated in `projectActions.ts` or the central watcher. The core and Live Remediation remain unaware of concrete Pipeline integrations.

  *Las integraciones de Pipeline utilizan ahora una **API interna de Integration Providers** bajo `src/modules/pipeline/integrations/`. La detección específica de cada herramienta, categorías, política de fallo, ayuda de instalación y archivos de configuración observados pertenecen a providers independientes y se componen mediante un registro, en lugar de enumerarse en `projectActions.ts` o en el watcher central. El core y Live Remediation siguen sin conocer integraciones concretas de Pipeline.*

- The **Integrations** tab now exposes normalized integration health metadata calculated in the Extension Host: functional category, installed/declared version when available, detection origins, configuration status, health, actual execution command, and setup guidance. Node setup/execution stays package-manager-aware, provider-contributed configuration files participate in live refresh, and the webview no longer owns a duplicated hard-coded tool catalog.

  *La pestaña **Integraciones** muestra ahora metadatos normalizados de salud calculados en el Extension Host: categoría funcional, versión instalada/declarada cuando está disponible, orígenes de detección, estado de configuración, salud, comando real de ejecución y ayuda de instalación. La configuración/ejecución Node continúa adaptándose al gestor de paquetes, los archivos aportados por cada provider participan en el refresco en tiempo real y el webview deja de mantener un catálogo duplicado y hardcodeado de herramientas.*

- Completed the documented **Integration/Pipeline roadmap (phases 1–16)**. Provider architecture, health, stack-aware recommendations, probes/setup, variables/secrets, preview/timings, structured results, provider parsers, structured history, differential findings, diagnostics, and lifecycle/concurrency hardening are all implemented while remaining inside the optional Pipeline module.

  *Se ha completado el **roadmap documentado de Integraciones/Pipeline (fases 1–16)**. La arquitectura de providers, salud, recomendaciones por stack, probes/instalación, variables/secretos, preview/duraciones, resultados estructurados, parsers, historial estructurado, diff de findings, diagnósticos y hardening de ciclo de vida/concurrencia están implementados manteniéndose dentro del módulo opcional Pipeline.*

- Added **structured Pipeline tool results and differential execution history**. Pipeline integration providers can attach normalized findings/metrics to a step without replacing its console output. ESLint, Node dependency audit, Semgrep, React Doctor, and Ruff now provide parsers; machine-readable output is requested where supported, snapshots are bounded before persistence, and history computes new/persistent/resolved findings against the latest compatible project/branch/tool/parser snapshot using stable fingerprints. The execution detail renders severity totals, metrics, and the structured diff while legacy history entries remain compatible.

  *Se han añadido **resultados estructurados de herramientas y diff entre ejecuciones de Pipeline**. Los providers pueden adjuntar findings/métricas normalizados a un paso sin sustituir su salida de consola. ESLint, auditoría de dependencias Node, Semgrep, React Doctor y Ruff incluyen parser; se solicita salida legible por máquina cuando está soportada, los snapshots se limitan antes de persistirse y el historial calcula findings nuevos/persistentes/resueltos contra el último snapshot compatible de proyecto/rama/herramienta/parser mediante fingerprints estables. El detalle de ejecución muestra totales por severidad, métricas y diff manteniendo compatibilidad con entradas antiguas.*

- Completed Pipeline **variables and secrets**. Steps can resolve `${workspaceFolder}`, `${projectKey}`, `${projectName}`, `${serverUrl}`, `${branch}`, `${packageManager}`, workspace `${variable.NAME}` values, and detected `${integration.<id>.command}` references. Secret values are captured through native VS Code password prompts, stored only in `SecretStorage`, injected through per-process environment variables, masked in preview, and included in output redaction without being sent to the webview, persisted in YAML/history, or stored in workspace state.

  *Se ha completado **variables y secretos** de Pipeline. Los pasos pueden resolver `${workspaceFolder}`, `${projectKey}`, `${projectName}`, `${serverUrl}`, `${branch}`, `${packageManager}`, valores de workspace `${variable.NOMBRE}` y referencias `${integration.<id>.command}` de integraciones detectadas. Los valores secretos se solicitan mediante prompts nativos de contraseña de VS Code, se guardan únicamente en `SecretStorage`, se inyectan mediante variables de entorno por proceso, se enmascaran en la previsualización y participan en la redacción de la salida sin enviarse al webview, persistirse en YAML/historial ni guardarse en workspace state.*

- Added **project stack detection and stack-aware integration recommendations** inside the Pipeline module. The Extension Host now recognizes Node/JavaScript, TypeScript, React, Vitest/Jest, Python/pytest, Java with Maven/Gradle, .NET, Go, Terraform, and Docker from workspace evidence. Integration providers declare open compatibility tokens and recommendation priority, so the Integrations tab shows only tools relevant to the detected stack, explains why each tool is recommended, and filters Available/Recommended lists by functional category. Stack-relevant files are also watched for live refresh without moving this logic into the core.
...
- Extended **Diagnostics** with generic module runtime/health contributions and detailed Pipeline integration metadata without statically importing optional module implementations or loading disabled modules. Provider and module diagnostic failures are isolated so one faulty contribution cannot break the complete report.
  ...
  *Se ha ampliado **Diagnóstico** con estado runtime/salud genérico de módulos y metadatos detallados de integraciones de Pipeline sin importar estáticamente implementaciones opcionales ni cargar módulos desactivados. Los fallos de providers y contribuciones de diagnóstico quedan aislados para que una integración defectuosa no rompa el informe completo.*

- Added historical Pipeline step timing statistics and pre-run estimates. The confirmation review now shows resolved step commands, failure policy, last/average timing when available, and an estimated total duration derived from compatible completed history entries.
