...
export function getDashboardLauncherHtml(
  ...
  language: DashboardLanguage
    ...
    const iconUri = (file: string) => webview.asWebviewUri(
      vscode.Uri.joinPath(extensionUri, 'assets', file)
    );
...
<body>
  ...
  <script nonce="${nonce}">
    ...
    function renderGate(element, status) {
      const normalized = dashboardConstants.qualityGateStatuses.includes(status)
        ? status
        : 'NONE';
      const labels = dashboardConstants.compactQualityGateLabels;
    ...
    function renderRating(element, grade) {
      const normalized = dashboardConstants.ratingGrades.includes(grade) ? grade : 'NONE';
